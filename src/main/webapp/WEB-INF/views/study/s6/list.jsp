...
<body>
    <div class="container mt-3">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
            <a href="/s6/findList" class="btn btn-secondary">전체</a>
            <a href="/s6/findList/1" class="btn btn-success">승인</a>
            <a href="/s6/findList/0" class="btn btn-warning">미승인</a>
            <a href="/s6/new" class="btn btn-primary">추가</a>
        </div>
        <div class="list-group mt-2 text-center">
        	<c:forEach items="${list}" var="row">
	            <a class="list-group-item m-1 ${(row.accept)? 'bg-success':'bg-warning' } display-6" href="/s6/detail/${row.no}">
	                ${row.title}
	            </a>
            </c:forEach>
        </div>
